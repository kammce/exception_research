...
public:
    ...
    : m_channel(p_channel)
  {
    if (m_channel >= 1'000'000'000) {
      checked_throw(my_error_t{ .data = { 0x55, 0xAA, 0x33, 0x44 } });
    }
  ...
  {
    if (m_channel >= 1'000'000'000) {
      checked_throw(my_error_t{ .data = { 0xAA, 0xBB, 0x33, 0x44 } });
    }
    side_effect = side_effect + 1;
  }
...
funct_group0_0()
{
  ...
  if (side_effect > 0) {
    start_cycles = uptime();
    checked_throw(my_error_t{ .data = { 0xDE, 0xAD } });
  }
